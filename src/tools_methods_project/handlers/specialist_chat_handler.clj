  ...
  (let [body        (slurp (:body request))
    ...
    (case action
      ...
      (if (and sender receiver)
        (let [result (chat/start-chat-session sender receiver)]
          (case (:domain-status result)
            ...
            {:status 500
             :headers {"Content-Type" "application/json"}
             :body (json/encode {:error "Unknown domain status."})}))
        {:status 400
         :headers {"Content-Type" "application/json"}
         :body (json/encode {:error "Sender and receiver are required to start a chat."})})
      ...
      (if (and sender receiver message)
        ...
        {:status 400
         :headers {"Content-Type" "application/json"}
         :body (json/encode {:error "Sender, receiver, and message are required to send a message."})})
      ...
      (if (and sender receiver)
        ...
        {:status 400
         :headers {"Content-Type" "application/json"}
         :body (json/encode {:error "Sender and receiver are required to show messages."})})

      {:status 400
       :headers {"Content-Type" "application/json"}
       :body (json/encode {:error "Invalid action."})})))